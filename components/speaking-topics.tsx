...
// da imagem muda — nenhuma alteração de caminho aqui). CTA "Ver mais",
// 1ª rodada: fonte sobe de text-sm para text-base ("deixe-o com a fonte
// um pouquinho maior").
//
// 2ª troca de capas + imagens de modal (Caio, 2026-09-23 — "troque as
// imagens atuais pelas imagens de mesmo nome do mesmo caminho... para
// os modais, use as imagens de mesmo nome... com o final ' - Sem
// Texto'"): tema-{1,2,3}.png atualizados de novo (mesmo nome de
// arquivo, conteúdo novo) e, desta vez, tema-{1,2,3}-sem-texto.png
// TAMBÉM são substituídos pelas variantes "- Sem Texto" — a nota da
// rodada anterior ("não foram tocadas") não se aplica mais aqui, Caio
// pediu explicitamente desta vez.
//
...
// sem apertar.
// Fundo dos modais (Caio, 2026-09-23 — "aplique um gradiente mais
// 'dinâmico', assim como os gradientes dos cards da sessão 'Por que
// agir agora'"): antes o `DialogContent` ficava com o
// `bg-[var(--surface-raised)]` sólido padrão (herdado de
// components/ui/dialog.tsx). Cada tema ganha o mesmo tipo de composição
// radial+linear já usada em why-now.tsx (`ReasonsGrid`/`REASONS`,
// reaproveitada aqui, não reinventada) — ângulo/foco do radial
// diferente por tema para não repetir o mesmo card 3 vezes.
...
const TOPICS = [
  {
    ...
    modalGradient: MODAL_GRADIENTS[0],
  },
  {
    ...
    modalGradient: MODAL_GRADIENTS[1],
  },
  {
    ...
    modalGradient: MODAL_GRADIENTS[2],
  },
] as const;
...
export function SpeakingTopics() {
  return (
    ...
    >
      ...
      <div className="px-6 pb-12 pt-4 sm:px-10 sm:pb-16 lg:px-12">
            ...
            {TOPICS.map((topic, index) => (
              <Reveal key={topic.value} index={index}>
                <Dialog>
                  ...
                  {/* V27 (Caio, 2026-09-23 — "pode diminuir UM POUCO seu
                      tamanho horizontal... não tem essa necessidade" +
                      "no fundo dos modais, aplique um gradiente mais
                      'dinâmico'"): largura desce de `min(1360px,94vw)`
                      para `min(1200px,90vw)` — redução modesta, o
                      suficiente para reduzir o vão vazio da coluna de
                      texto sem apertar o conteúdo (mesma altura,
                      mesma proporção 35/65 de colunas). `bg-transparent`
                      substitui o `bg-[var(--surface-raised)]` sólido
                      padrão de ui/dialog.tsx (dedupe via `cn`/twMerge no
                      próprio DialogContent) — o gradiente real vem do
                      `style` inline (`topic.modalGradient`, ver
                      MODAL_GRADIENTS acima), que vence sobre a classe
                      porque `background` inline sempre tem prioridade
                      sobre `background-color` de uma classe. */}
                  <DialogContent
                    className="h-[min(88vh,800px)] w-[min(1200px,90vw)] max-w-none overflow-hidden bg-transparent p-0"
                    style={{ background: topic.modalGradient }}
                  >
